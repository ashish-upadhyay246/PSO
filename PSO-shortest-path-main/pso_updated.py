...
def plot_circle(circle): #to plot the circular obstacles
    plt.gca().add_patch(
        plt.Circle(
...
def plot_graph(best_location, initial_particles):
    plt.close("all")
    ...
    # Plot initial particles with gray color
    initial_particles_x = [particle[0] for particle in initial_particles]
    initial_particles_y = [particle[1] for particle in initial_particles]
    plt.scatter(
        initial_particles_x,
        initial_particles_y,
        10,
...
def random_initialization(swarm_size): #random initialzatin of particles.
    """
...
def loss_function(x): #fitness/objective function
    """
...
DIM = 11 #points to be considered in the final path, higher the number = better curves = higher computational compexity.
START = ob.Point(1, 1)
END = ob.Point(20,20)
...
def main():
    ...
    obstacle1 = ob.Obstacle_Circle(2, ob.Point(9, 14))
    obstacle2 = ob.Obstacle_Circle(3.5, ob.Point(5, 6))
    obstacle3 = ob.Obstacle_Circle(2, ob.Point(11.5, 9))
    obstacle4 = ob.Obstacle_Circle(3, ob.Point(15, 15))
    obstacles = [obstacle1, obstacle2, obstacle3, obstacle4]

    particles_loc, _, _, _, _, _ = random_initialization(100)
    initial_particles = particles_loc[:, 0, :].tolist()
    ...
    print("100% completed!")
    plot_graph(best_location, initial_particles)
